...
PR1–PR23 are complete in the intended post-merge repository state. The repository includes canonical domain contracts, content-addressed local storage and provenance, deterministic timeline compilation, the generic FFmpeg backend, durable preview/final render orchestration, authenticated local FastAPI and Inbox ingest, the installable phone-first PWA/share flow, the review/proxy-preview/final-production lifecycle, the exact-version template/component/skin registry boundary, the first registered non-voiced template pack, reusable semantic overlay/motion components, deterministic music/audio composition with lossless premaster caching, two-pass loudness mastering and audio QC, the optional LLM provider boundary backed by `chatgpt-web-adapter` with proposal-only review authority, language variants with localized metadata/font intent plus deterministic variant-specific preview/final cache identity, the v0.1 batch-production boundary with crash-safe frozen-plan recovery and reproducibility evidence, retained panel OCR/correction authority, explicit dialogue reading order and speaker assignment with accepted-state provenance revalidation, a durable per-line TTS layer with replaceable provider contracts, pinned Qwen3-TTS integration, content-addressed generated audio, exact cache identity, independently verified WAV evidence, a persistent Voice Cast registry that keeps narrative character identity separate from reusable voice identity while pinning immutable cast revisions and reference-audio evidence, a derived voiced-story layer that materializes verified dialogue WAVs plus deterministic phrase timing into the shared Scene/timeline runtime with reversible ownership and fail-closed render validation, and a reversible voiced-scene presentation layer for deterministic pause/overlap QC, music/ambience ducking, profile-independent camera focus intent, authenticated presentation review, and exact-snapshot render authority.

The current implementation step is **PR24 — Long-form output profiles** in **Milestone 6 — Long-form and reusable production assets**. PR24 extends the existing scene/timeline/render architecture to 16:9 1080p/1440p output, chapter/scene concatenation, shared voiced scenes across Short and long-form projects, and long-form render caching without introducing a separate renderer.
...
Optional language/semantic assistance is available through the PR15 provider boundary, PR16 can compile multiple localized variants from one shared source/timeline graph, and PR17 can prepare/render/QC those outputs as authenticated durable batches. **Milestone 5 is complete through PR23**: retained panel OCR, accepted dialogue reading order/speaker identity, durable per-line TTS with pinned local Qwen support, persistent reusable Voice Cast identity, materialized voiced-story timing/text/audio, and reversible voiced-scene mix/camera presentation all use the same canonical Scene/timeline runtime. Milestone 6 now extends that same runtime toward long-form output rather than creating a parallel production stack.
...
- [`docs/pr23-voiced-scene-mix-camera-choreography.md`](docs/pr23-voiced-scene-mix-camera-choreography.md) — reversible voiced-scene presentation plans, retained-state hardening, camera/mix FFmpeg compilation, authenticated review surface, and fail-closed render lifecycle

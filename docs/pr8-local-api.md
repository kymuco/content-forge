...
The staging pathname is an opaque internal recovery key. It always uses a fixed `.upload` suffix and never incorporates the user-controlled filename extension, so long, multibyte, or platform-invalid filename suffixes cannot create an oversized/invalid filesystem component. The exact original filename is retained only as intake/provenance/project metadata and remains available to the UI and later source logic.
...
A media-content preparation failure does not delete an already accepted immutable asset. ffprobe failures and bounded FFmpeg thumbnail-generation outcomes such as nonzero exit, timeout, missing/empty output, or failure to launch the configured FFmpeg executable produce a path-safe partial result while retaining the project/asset identity. Operational filesystem/SQLite failures after FFmpeg has launched are not folded into that `partial` category; after FULL acceptance they remain `receiving` and retryable under the storage policy above.
...
URL/note intake deliberately does not create a fake `Asset` or fake `SourceRecord` when no bytes exist. It creates a durable Inbox intake plus an assetless `Project(INBOX)` whose metadata preserves the captured URL/note/creator hint. Its JSON body is bearer-authenticated before parsing, requires a valid `Content-Length`, and is capped at 128 KiB at the transport boundary. A future source provider can resolve bytes later without changing this boundary. Filesystem or SQLite operational failures while creating/linking the deterministic project or committing the terminal receipt leave the intake `receiving` with a retryable diagnostic; exclusive startup reconciliation reuses the same project identity and finishes once storage recovers instead of forcing a duplicate intake/project. Process shutdown signals likewise propagate without converting an otherwise recoverable `receiving` URL/note intake into `failed`.
...
Thumbnail outcome classification separates execution/generation failure from storage failure. Nonzero exit, timeout, missing/empty output, or an `OSError` raised specifically while launching the configured FFmpeg executable becomes `thumbnail_failed` and may yield a terminal `partial` intake. Filesystem/storage `OSError` after launch during thumbnail output handling, publication, receipt writing, or durability is allowed to propagate unchanged so the FULL-accepted intake remains `receiving` and is retried after storage recovers.

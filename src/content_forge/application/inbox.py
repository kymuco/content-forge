            ...
            if not lookup_failed and current is not None and current.state is IntakeState.RECEIVING:
                ...
                if not isinstance(exc, Exception):
                    ...
                    if accepted and (retryable_operational or not project_checkpointed):
                        ...
                        # staging remains the recovery authority. Crucially, this path
                        # re-raises instead of returning RECEIVING as a synthetic success,
                        # so the PWA keeps its only queued retry copy.

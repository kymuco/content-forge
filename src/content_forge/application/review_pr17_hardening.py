...
PR10's public class identities are already compatibility surfaces. This module upgrades
only the inherited reconciliation method in place so existing imports and monkeypatch
points remain stable while PR17 batch attempts keep exclusive lifecycle ownership.
"""
...
def _reconcile_persisted_state(self: _review.ReviewService) -> None:
    """Recover PR10 state without stealing PR17-owned render attempts.

    Presence of ``batch_context`` reserves render-attempt lifecycle authority to PR17.
    Even malformed batch context is left untouched here so the batch layer can fail
    closed on its own integrity contract instead of PR10 rewriting the attempt state.
    """
    ...
    for project in projects:
        ...
        except _review.ReviewConflictError:
            continue
        valid_projects.append(project)
        valid_ids.add(project.project_id)

    # Reset only validated PR10 preview claims. This changes Project review state, not
    # render-attempt ownership, and therefore remains valid when the same Project also
    # has independent PR17 batch work.
...
def install_pr17_review_reconciliation() -> None:
    """Install the ownership-aware reconciler without changing ReviewService identity."""

    _review.ReviewService.reconcile_persisted_state = _reconcile_persisted_state


install_pr17_review_reconciliation()

__all__ = ["install_pr17_review_reconciliation"]
